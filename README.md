# csv_median_calculator v2.0 (C++23)

**Реализация ТЗ:** Инкрементальная медиана O(log N) из CSV потоков биржевых торгов
...
firefox docs/html/index.html  # Просмотр
```
...
4. **Чтение и объединение данных**: считывание всех подходящих CSV-файлов. Если файл имеет некорректный формат CSV (невалидный заголовок, повреждённые данные, отсутствующие обязательные колонки), файл целиком исключается из обработки с логированием ошибки — источник данных считается невалидным
5. **Сортировка данных** по полю `receive_ts` с использованием `std::stable_sort` для сохранения относительного порядка записей с одинаковым `receive_ts`
...
1. Задать вопрос по электронной почте
2. Связаться с рекрутером
