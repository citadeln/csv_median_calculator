...
StatsCalculator::StatsCalculator(const std::vector<std::string>& metrics) 
    ...
    for (const auto& m : metrics) {
        ...
        else if (m == "median") headers_.push_back("median");
    }
}
...
std::vector<std::optional<double>> StatsCalculator::get_stats() const {
    ...
    // Boost.Accumulators НЕ ТРЕБУЕТ count() проверки
    for (const auto& m : metrics_) {
        if (m == "mean") {
            stats.push_back(mean(acc_));
        } else if (m == "std") {
            stats.push_back(sqrt(variance(acc_)));
        } else if (m == "p50" || m == "median") {
            stats.push_back(median(acc_));
        } else if (m == "p90") {
            stats.push_back(quantile(acc_, quantile_probability<0.9>()));
        } else if (m == "p95") {
            stats.push_back(quantile(acc_, quantile_probability<0.95>()));
        } else if (m == "p99") {
            stats.push_back(quantile(acc_, quantile_probability<0.99>()));
        }
    }
    return stats;
}
